...
public sealed class FakeGraphStore : IGraphStore
{
  ...
  public Task<Path?> FindShortestPathAsync(int characterAComicVineId, int characterBComicVineId, int maxDepth)
  {
    ...
    while (queue.Count > 0)
    {
      ...
      if (current == characterBComicVineId)
      {
        return Task.FromResult<Path?>(ReconstructPath(characterAComicVineId, current, parent));
      }
  ...
  public Task<IReadOnlyDictionary<int, IReadOnlyList<int>>> FindOverlappingIssuesAsync(int comicVineId,
  ...
  private Path ReconstructPath(int startId, int endId, Dictionary<int, int> parent)
  {
    ...
    for (var i = 0; i < idPath.Count - 1; i++)
    {
      ...
      hops.Add(new Hop(characters[i], characters[i + 1], connection.ComicIssueId, connection.Tier,
        connection.Confidence, connection.ComicIssueName, connection.ComicIssueSiteDetailUrl));
    }
