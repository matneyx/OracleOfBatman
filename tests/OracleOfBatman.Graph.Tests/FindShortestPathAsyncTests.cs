...
public sealed class FindShortestPathAsyncTests : IAsyncLifetime
{
    ...
    public async Task ReturnsNull_WhenEitherCharacterDoesNotExist()
    {
        ...
        var path = await _writer.FindShortestPathAsync(12605, 999999, maxDepth: 5);
    ...
    public async Task ReturnsNull_WhenNoPathExists()
    {
        ...
        var path = await _writer.FindShortestPathAsync(12605, 157242, maxDepth: 5);
    ...
    public async Task ReturnsOneHopPath_ForDirectConnection()
    {
        ...
        await _writer.UpsertConnectionAsync(new Connection(12605, 157242, 739613, null, InteractionTier.SameIssue, Confidence.Unverified));
    ...
    public async Task ReturnsHopsInWalkOrder_RegardlessOfStoredRelationshipDirection()
    {
        ...
        await _writer.UpsertConnectionAsync(new Connection(15694, 176719, 111, null, InteractionTier.SameIssue, Confidence.Unverified));
        await _writer.UpsertConnectionAsync(new Connection(15734, 15694, 222, null, InteractionTier.SameIssue, Confidence.Unverified));

        var path = await _writer.FindShortestPathAsync(176719, 15734, maxDepth: 5);
    ...
    public async Task ReturnsCharacterImageSiteDetailUrl_AndHopIssueNameAndSiteDetailUrl()
    {
        ...
        var jeff = new Character(157242, "Jeff the Land Shark", "https://example.com/jeff-icon.jpg", "https://comicvine.gamespot.com/jeff-the-land-shark/4005-157242/");
        await _writer.UpsertCharacterAsync(jimHammond);
        await _writer.UpsertCharacterAsync(jeff);
    ...
    public async Task ReturnsNull_WhenShortestPathExceedsMaxDepth()
    {
        ...
        await _writer.UpsertConnectionAsync(new Connection(1, 2, 111, null, InteractionTier.SameIssue, Confidence.Unverified));
        await _writer.UpsertConnectionAsync(new Connection(2, 3, 222, null, InteractionTier.SameIssue, Confidence.Unverified));

        var path = await _writer.FindShortestPathAsync(1, 3, maxDepth: 1);
